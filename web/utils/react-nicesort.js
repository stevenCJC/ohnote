(function () {
	...
	var getReorderComponent = function (React) {
		...
		return React.createClass({
			...
			onMouseMove : function (event) {
				...
				if (this.state.held && this.state.dragged) {
					...
					if (collision) {
						...
						//更新状态，以更新节点
						//这里可以切入动画动作，结束后调用setState
						//事件：即将删除Class:aboutToDelete，即将改变位置Class:hovered，callback
						/* this.animationMove(this.state.dragged.target,collision,2000,function(){
							this.setState({
			...
			//只支持单行或单列，单方向锁定
			animationMove:function(target,collision,wait,callback){
				console.log({target,collision});
				collision.classList.add('animation-move');
				//target.classList.add('animation-move');
				setTimeout(function(){
					collision.classList.remove('animation-move');
					callback();
				},wait);
			},
